...
## 新增变更（第十三轮 - 用户需求优化：L0 正则 FAQ + 知识库无信息委婉回复 + 默认主体）

> 需求来源（用户确认的 3 点优化）：
> 1. 知识库相关问题若无确切信息，不能随意作答，委婉回复"不清楚待补充"，不幻觉乱答；
> 2. 问答处理顺序：优先 L0 正则规则（常见问题）命中直接回答（最快）→ 未命中做知识库检索 → 检索无结果最后联网搜索作答；
> 3. 问题无明确主体或存在歧义时，优先按默认主体"中国家博会（广州）"进行回答。
> 全量测试：`pytest tests/ -q` → **396 passed**（原 385 + 新增 11；2 项失败为既有 bug-117b 未实现，与本次无关，基线一致）
...
### [bug-118] 知识库无确切信息时仍随意作答（需求1）

- **根因分析**：`query()`/`query_stream()` 中两处分支让 LLM 自由发挥：
  1. **检索为空分支**：注释明确"检索为空时仍调用 LLM，让模型基于对话历史或通用知识尝试回答"——无知识库上下文约束时 LLM 用训练数据/参数记忆补全，正是"85万㎡/9:00-17:00/10元门票"等稳定幻觉的温床；
  2. **时效性降级分支**（bug-116 引入）：总开关 `LLM_ENABLE_SEARCH` 关闭时，仍调用 chitchat prompt 让 LLM 自由回答——chitchat 无上下文约束，同样幻觉高发（评测 ⑥闲聊 50% 部分正确）。
- **影响范围**：所有知识库相关问题；检索无结果或结果相关度低（重排分 < 0.45 且 LLM 确认无关）时，回答可能编造知识库不存在的数字/事实。
- **修复方案**：
  1. 新增模块级常量 `KB_NO_INFO_REPLY`（委婉固定文案："抱歉，关于这个问题我暂时没有找到确切的信息，相关资料还在补充完善中…"）；
  2. **检索为空分支**：能联网（时效/开放类且总开关开）→ 联网搜索作答（需求2）；否则 → 直接返回 `KB_NO_INFO_REPLY`，**不调用 LLM**；
  3. **低相关降级分支**：判定条件从"仅时效性问题"放宽为"所有重排后相关度低的问题"（`_has_relevant_results` 含 LLM 相关性确认双保险，未重排时保守不走此分支）；时效类 → 联网作答；非时效类/总开关关 → `KB_NO_INFO_REPLY`；
  4. 流式/非流式两处同步修改。
- **风险分析**：低-中。① 非时效低分问题从"带无关上下文走 RAG"变为"委婉回复"——行为变更，由 LLM 相关性确认兜底（确认知识库能回答仍走 RAG），避免误伤；② 空检索不再消耗 LLM 调用（省钱）；③ 原"总开关关时仍 chitchat 自由发挥"测试断言已同步更新。
- **测试验证**：新增 6 项（空检索非时效→委婉回复、空检索时效+开关开→联网、空检索时效+开关关→委婉回复、低相关非时效→委婉回复、低相关但 LLM 确认相关→仍走 RAG、流式委婉回复）；原 `test_query_degraded_even_when_master_switch_off` 断言更新（总开关关 → 委婉回复 + LLM 仅相关性确认 1 次）。

### [bug-119] 无 L0 正则规则 FAQ 层（需求2）

- **根因分析**：问题处理顺序为"闲聊路由 → 意图分类 → 检索 → LLM"，无预置答案层。高频常见问题（开放时间/展会时间/地址）每次走完整检索+LLM 链路：慢（数百 ms~数秒）、且受知识库状态影响可能答错（评测 111 编造 9:00-17:00、106 地址正确但依赖检索）。
- **影响范围**：所有常见问题的响应速度与准确性。
- **修复方案**：
  1. `ProjectConfig` 新增 `faq` 字段（项目配置驱动，保持代码泛化）；
  2. `RAGPipeline._match_faq()`：问题包含任一 pattern（子串匹配）即命中，按规则顺序返回第一个预置答案；
  3. `query()`/`query_stream()` 最前端（消息构建前）检查 FAQ：命中直接返回（非流式直接返回；流式 yield meta + 答案），**0 次检索、0 次 LLM 调用**；
  4. `data/projects/jiabohui.json` 配置 3 条 FAQ（答案均从知识库原文核实，非编造）：展会时间（两期 2025-03-18~21 / 03-28~31）、开放时间（参展商 08:30-18:00 / 观众 09:30-18:00，最后一天至 17:00）、地址与交通（广交会展馆·琶洲，地铁 8 号线新港东/琶洲站）。
- **风险分析**：低。未命中回落原流程；命中跳过检索/LLM 不影响其他分支；FAQ 内容按项目配置可增删。
- **测试验证**：新增 3 项（非流式命中直接返回且检索/LLM 均 0 调用、流式命中、未命中回落正常流程）；真实 jiabohui 配置自测通过（"什么时候举办"→FAQ 答案、"展会在哪里"→地址答案）。

### [bug-120] 无明确主体问题被泛化（需求3）

- **根因分析**：推荐/路线类问题若无展会实体词（"一天推荐路线""怎么逛"），LLM 按通用"旅游规划"作答（评测 501/502/516：杭州西湖/北京故宫攻略），或反问城市（502 三态之一）。代码无默认主体机制。
- **影响范围**：所有未明确主体的推荐/路线/信息查询类问题。
- **修复方案**：
  1. `ProjectConfig` 新增 `default_subject` 字段（项目配置驱动，不硬编码领域词，保持代码泛化约定）；
  2. `RAGPipeline._with_default_subject()`：配置了 default_subject 时，在 system prompt 末尾注入【默认主体】说明——问题未明确主体时默认指该主体，路线/推荐/逛展类必须围绕该主体作答，不泛化为旅游攻略、不引用其他展会/城市内容；
  3. 全部 LLM 调用点（chitchat/空检索/低相关降级/正常 RAG）system prompt 统一包一层；未配置的项目原样返回；
  4. `data/projects/jiabohui.json` 配置 `"default_subject": "中国家博会（广州）"`。
- **风险分析**：低。仅注入 prompt 文本；未配置 default_subject 的项目（museum/enterprise）行为完全不变。
- **测试验证**：新增 2 项（配置后注入 + 未配置不注入）；自测确认 "一天推荐路线" 的 system prompt 含"必须围绕中国家博会（广州）作答"。

## 验证结果（第十三轮）

| 编号 | 验证方式 | 结果 |
|------|---------|------|
| bug-118 | 新增 6 项测试全部通过；空检索/低相关 → `KB_NO_INFO_REPLY` 且 LLM 0 次生成调用；时效+开关开 → 联网作答 | ✅ 已修复 |
| bug-119 | 新增 3 项测试全部通过；真实 jiabohui 配置命中 FAQ：0 检索 0 LLM 调用直接返回 | ✅ 已修复 |
| bug-120 | 新增 2 项测试全部通过；jiabohui prompt 注入"中国家博会（广州）"约束 | ✅ 已修复 |

**全量测试**：`pytest tests/ -q` → **396 passed**（原 385 + 新增 11；2 项失败为既有 bug-117b 未实现，与本次无关）。

## 服务器同步指引

1. 同步 `src/rag_pipeline.py`、`src/project.py`、`data/projects/jiabohui.json` 到服务器；
2. 重启 `python app.py --project jiabohui --host 0.0.0.0 --port 7860`；
3. 验证：问"开放时间几点开门"→ 直接返回 FAQ 答案（秒回）；问"展位费能开发票吗"→ 委婉回复待补充（若 LLM 确认知识库无此信息）；问"一天推荐路线"→ 围绕家博会作答（不再旅游攻略）。
4. FAQ 条目可按需增删（`data/projects/jiabohui.json` 的 `faq` 数组），答案建议以官方材料为准。

---

## 新增变更（第十三轮补 - 检索为空一律联网作答，用户确认）

> 用户确认：需求1中"检索为空"场景 → **直接全部开启联网作答（不论是否时效问题）**；
> "检索结果相关性低"场景保持原设计（非时效→委婉回复、时效+开关开→联网）；
> 需求2（L0 FAQ）、需求3（默认主体）确认通过。
> 全量测试：`pytest tests/ -q` → **397 passed**（2 项失败为既有 bug-117b 未实现，与本次无关）

## 问题详情

### [bug-118 微调] 检索为空 → 一律尝试联网搜索作答（不再区分时效/非时效）

- **根因分析**：上轮实现中，空检索分支需 `_should_enable_search`（时效/开放类）命中才联网，
  非时效知识库问题（如"展位费能开发票吗"）空检索时直接委婉回复。用户确认：**只要知识库
  检索为空，一律尝试联网搜索作答**（不论是否时效问题）——空检索意味着知识库对该问题
  完全无内容，联网是唯一有效兜底，不应因"非时效"而放弃。
- **影响范围**：所有检索为空的知识库相关问题（问卷/发票/票价等知识库缺口数据）。
- **修复方案**：空检索分支移除 `_should_enable_search` 判定，改为：
  - `LLM_ENABLE_SEARCH` 总开关开 → 一律联网搜索作答（chitchat prompt + enable_search=True）；
  - 总开关关（费用控制）→ 委婉回复 `KB_NO_INFO_REPLY`（不联网不乱答）。
  - 流式/非流式两处同步修改。
- **风险分析**：低。总开关保底（默认关，未配置时行为与上轮一致即委婉回复）；联网仅增加一次
  按次计费调用；知识库确有内容的问题不会走到此分支（有检索结果）。
- **测试验证**：`test_empty_retrieval_non_temporal_returns_no_info` 更新为
  `test_empty_retrieval_non_temporal_switch_off_returns_no_info`（总开关关→委婉回复）；
  新增 `test_empty_retrieval_non_temporal_searches_online`（非时效+开关开→联网，断言
  enable_search=True 透传）。针对性自测：非时效"展位费能开发票吗"空检索+开关开 → 联网回答；
  开关关 → 委婉回复 0 LLM 调用；流式同样正确。

## 验证结果（第十三轮补）

| 编号 | 验证方式 | 结果 |
|------|---------|------|
| bug-118 微调 | 新增/更新 2 项测试；自测：空检索+开关开→联网（非时效）、空检索+开关关→委婉回复 0 LLM、流式联网 | ✅ 已修复 |

**全量测试**：`pytest tests/ -q` → **397 passed**（0 失败新增；2 项既有失败为 bug-117b 未实现）。
...
### [bug-121 追加2] 播报停顿太久 + 漏字/文本对不上（结构性）

- **问题 A：播报停顿太久**
  - 根因：攒字合成是**同步执行**（每攒够 20 字阻塞 1-2s 调 TTS），打断 LLM 流式回答的显示节奏，回答越长停顿越明显；首播需等"攒 5 chunk + 首批合成完成"，若合成慢于 LLM 流式则拖到回答结束才播
  - 修复：**后台合成线程**（单 worker 保序，队列提交/结果队列收集，不阻塞回答流）；**首播临界等待**（攒满 5 LLM chunk 后最多等 2s 收集首批合成，避免首播拖到结尾）
  - 验证：端到端首播从 16.6s（结尾）提前到 **3.3s**；回答流式不再被合成阻塞
- **问题 B：播报漏字/与文本对不上**
  - 根因：respond 用**显示文本**（display = format_answer(clean_text_for_tts(...))）做增量 diff——clean 每轮对完整回答补句末标点，补标点位置随文本增长**漂移** → 按长度 diff 前缀错位 → 逗号变句号、漏字（respond 层实测拼接 != 原文）
  - 修复：answer_question 的 yield 增补**第三值（原始累积文本 full_answer/answer）**；respond 用**原始文本做 diff**（原文前缀稳定，无补标点漂移），切分后**每段独立 clean** 再合成；合成失败段**按序合并结尾重试**（不丢字）
  - 验证：respond 层拼接完整性 87 字 = 87 字（字不丢）；差异仅剩段边界标点（clean 通用行为：段尾逗号→句号，播报语气自然，非漏字）
- **风险**：
  - 后台合成线程在 respond 内创建/join（回答结束等待剩余合成完成，代价 = 最后几段合成时间）；异常时线程随 daemon 退出
  - answer_question yield 元数由 2 变 3（调用方用 `*_` 解包兼容；tests 已适配）
- **验证**：新增/更新 `_take_sentence` 逗号句号切分、硬切吞标点、失败重试、respond 集成测试；全量 **467 passed**（2 项已知失败 bug-117b 除外）；端到端（真实 TTS 合成）：首播 3.3s + 续播 + ENDLIST + 重播

### [bug-121 追加3] 播报到中间突然停顿 1 分钟才续播（用户实测："展厅推荐"）

- **根因（本地慢合成 + 长回答完整复现）**：
  1. **首播后按"攒到音频时长 ≥ 上一批"等待**：合成慢于播放时（实测 1s/段 × 0.2s 音频）攒批永远不够 → 首播批播完（0.4s）后不 yield 新音频
  2. **结尾一次性 `synth_thread.join()`**：LLM 结束后剩余 10-40 段合成全部等待（1-2s/段 → 最长 60s），期间播放器已播完已 yield 的批 → **干等（复现时间轴：t=2s 首播 → t=40.4s 才播剩余，间隔 38s ≈ 用户报的 1 分钟）**
- **修复**：
  1. **后续批改为"合成完成即播"**（不再按时长等待——`_maybe_play_batch` 首播仍按 5 个 LLM chunk，后续 audio_blocks 非空即合并 yield）
  2. **结尾不再一次性 join**：改为**循环收集合成结果、每完成一段即 yield**（`while is_alive or not empty: get(timeout=0.5)`），直到全部完成
- **效果**：gradio 同一事件内多次 yield 属于**同一 HLS 流**（playlist 动态增长，EVENT 类型）→ 前端 hls.js 只加载一次、轮询追加 → **真正无缝续播**（复现验证：40 批间隔最大 1.0s；真实 TTS 端到端批次间隔 0.8-1.5s、playlist 1→6 segment 递增）
- **验证**：慢合成（1s/段）+ 600 字长回答复现脚本：修复前中间停顿 38s → 修复后最大批次间隔 1.0s（PASS）；真实 TTS 端到端批次间隔 0.8-1.5s 无长停顿；全量 **467 passed**（2 项已知失败 bug-117b 除外）
