...
- **唤醒应答提速至 ~0.3s（web-048，用户拍板内核微调）**：应答语本就有缓存/预热，
  慢在唤醒判定——待机部分结果提前命中的评估窗口仅覆盖 `in_speech` 期间，讯飞完整
  部分结果多在说话结束后、VAD 端点闭合前才到齐 → 必落「端点 500ms+finish」慢通道
  （实测 +1.22s）。修复：`voice_assistant.py` 一处条件——评估窗口放宽为「ASR 会话
  存活期间」（匹配规则不变，不产生新误唤醒类别，其他内核模块零影响）。
  真实链路四轮实测 Δ=0.27/0.24/0.25s（1 轮讯飞抖动 1.24s 如实说明）。
- **返回中断播报（web-047，用户反馈）**：聊天态「返回」此前只跳页不停播——
  现先 `player.stop()` 本地立即静音（不等回包），再 `barge()` 通知服务端取消本轮。
- **答案触上限硬截断修剪（web-049，用户反馈截图实证）**：长答案被 320 tokens 钳制
