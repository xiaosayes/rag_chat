...
```
用户输入 → [0] 消息构建(历史8轮+当前问题)
   │
   ▼
[1] 路由判断(is_kb_related, L0规则闲聊路由)
   ├── 闲聊/非知识库 → [闲聊分支] chitchat prompt + 按需联网 → 直接 LLM（无检索）
   └── 知识库相关 → [2] 意图分类(分层: L1语义→L2 LLM→L0规则评分)
                    │   产物: query_type ∈ {推荐/事实/比较/开放/闲聊}
                    ▼
        [3] 混合检索(并行) ─┬─ 语义: Embedding → Qdrant
                            └─ BM25: rank-bm25 (内存索引)
                    │  RRF融合(语义0.7+BM25 0.3) + 去重
                    ▼
        [4] 重排序 (Qwen3-Reranker；时效问题强制重排，其余>3条才重排)
                    │
                    ▼
        [5] 降级决策: 需联网(层1关键词/层2语义) 且 知识库无相关内容(层3结果侧LLM确认)
                    │   → 降级: chitchat prompt + 联网（不携带无关上下文）
                    ▼
        [6] 构建上下文(去重+裁剪 ≤ 10000字符) + [7] 选择Prompt(按项目/意图)
                    │
                    ▼
        [8] LLM生成 (qwen-plus 流式/非流式；联网引导+日期注记+响应缓存)
                    │
                    ▼
        [9] 防幻觉检查(仅日志) → [10] 输出 + 检索结果可视化
```

> **完整分步说明（含各分支、prompt 选择、联网决策三层、相关 bug 索引）见：**
> `docs/superpowers/specs/2026-08-07-routing-flow.md`
