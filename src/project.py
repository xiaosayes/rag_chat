...
3. **相关性优先**：只推荐与用户问题**直接相关**的项；参考信息中与用户需求不相关的项**不要推荐**（宁缺毋滥，不要为凑满数量硬推）（bug-112）
4. 尽量覆盖 **不同类型**（文物、展览、主题活动等）
5. 推荐理由要具体，说明该结果为何值得关注
...
## 输出格式要求
请使用结构化格式输出推荐结果。
8. 回答必须一次完成：**列出全部推荐项后直接结束**，不要重复推荐、不要追加与前面相同的推荐列表，不要在结尾再次生成新的推荐内容（bug-102 防循环）
""",
