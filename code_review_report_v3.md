...
**web-072 续二（验收回归）**：浮层太窄（≈1 行）根因=向上可扩展空间被首页面板顶边卡死
（~229px）。改 **fixed 定位**（相对视口，彻底逃脱 .panel overflow 裁剪），底部锚候选条上方、
横向对齐键盘面板、目标高 540px（≈4 行+滚动，顶留 280px 鹿景余量）；键盘键高 108→88、
键字 42→34、候选字 48→40（用户拍板缩 1~2 号让位浮层）。vitest 120 / build ✓。

### 增补：web-074 意图分层闸（2026-08-22，真实 API 冒烟验证）

用户反馈：纯关键词正则意图识别薄弱易误判，要求「只要意图是讲故事，不管怎么提问都要识别」。

- **分层闸 resolve_story_intent**：①meta 元问题拦截（会不会讲/什么是/讲了什么→问答）；
  ②正则快路径（明确指令零延迟）；③安全问答信号（几点/多少/为什么/哪些/哪里/预约/活动
  →问答，不进 LLM）；④**LLM 兜底分类**（deepseek-v4-flash-0731，enable_thinking=False，
  max_tokens 80，6s 超时，故障回退问答）——模糊句式泛化识别并顺带提取主题（比正则抠词准）。
  prompt 含「你会讲西游记吗=story（点名）vs 你会讲故事吗=qa（未点名）」区分条款（实测踩坑后补）。
- **真实 API 冒烟 12 句式全对**：我想听嫦娥奔月/给我讲讲后羿射日吧/来一个三只小猪/
  有没有关于恐龙的故事/你会讲西游记吗→全部正确识别+主题；你会讲故事吗/恐龙有哪些种类/
  嫦娥奔月讲了什么/能给我唱首歌吗→正确走问答。LLM 路径延迟 2.0~2.7s（仅模糊句式承担，
  正则命中与明显问答零新增延迟）。
- **评审修复**：meta 拦截收窄语境（会不会/能不能须邻接讲说、什么是须句首——防误杀
  「丑小鸭会不会变天鹅」类故事主题）+非法 intent 枚举测试。**遗留 backlog**：无主题故事请求
  （「随便讲一个」）仍落问答，可后续对接 presets 随机主题。
- pytest **873 passed**（+17）。
